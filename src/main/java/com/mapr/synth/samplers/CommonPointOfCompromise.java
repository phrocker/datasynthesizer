...
/**
 * In this model, users do some number of transactions, the rate for which is selected from a Gamma distribution. The
 * merchants for the transactions are chosen from a long-tailed distribution so there are a few very common merchants
 * and a few less common ones. All users generate events for the entire period of data generation.
 * <p>
 * Users also visit ATM's. For a period of time, one ATM is "compromised" and all users who visit that ATM during that
 * time will be switched to the vulnerable state. During the vulnerable period, transactions have a higher probability
 * of being marked as "possible fraud". The vulnerable period starts a short period of time after the compromise and
 * extends a short period after that. Transactions at a compromised ATM are marked.
 * <p>
...
class CommonPointOfCompromise extends FieldSampler {
    ...
    private final Gamma transactionsPerDay = new Gamma(2, 1, gen);

    private final PittmanYorProcess merchant = new PittmanYorProcess(100, 0.3);
